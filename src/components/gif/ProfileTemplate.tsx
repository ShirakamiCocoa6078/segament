// 파일 경로: src/components/gif/ProfileTemplate.tsx
...
export const ProfileTemplate = ({ profile, honorToShow }: ProfileTemplateProps) => {
  ...
  // 외부 CSS 파일을 로드하기 위한 링크
  const fontStyles = `
    body { margin: 0; }
    div, span, img { position: absolute; }
  `;
        ...
        <div style={{ width: '1823px', height: '722px', position: 'relative', overflow: 'hidden' }}>
            ...
            {profile.teamName && (
                <div style={{ width: '1176px', height: '208px', left: '549px', top: '20px', backgroundRepeat: 'no-repeat', backgroundPosition: 'center', backgroundImage: `url(https://new.chunithm-net.com/chuni-mobile/html/mobile/images/team_bg_${profile.teamEmblemColor}.png)`}}>
                    <div style={{ width: '878px', height: '113px', left: '197px', top: '38px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontSize: '96px', ...msPGRFont }}>{profile.teamName}</div>
                </div>
            )}
            ...
            <div style={{ width: '731px', height: '96px', left: '748px', top: '363px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontSize: '96px', ...msPGRFont }}>{profile.playerName}</div>

            <div style={{ width: '160px', height: '128px', left: '549px', top: '378px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontSize: '64px', ...msPGRFont }}>Lv.</div>
            <div style={{ width: '160px', height: '144px', left: '634px', top: '361px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontSize: '96px', ...msPGRFont }}>{String(lv).padStart(2, '0')}</div>
            {star !== null && (
              <div style={{ position: 'relative', left: '532px', top: '307px' }}>
                <img style={{ width: '125px', height: '125px' }} src="https://chunithm-net-eng.com/mobile/images/icon_reborn_star.png" />
                <div style={{ width: '112px', height: '96px', left: '15px', top: '15px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontSize: '96px', ...msPGRFont }}>{star}</div>
              </div>
            )}

            <div style={{ left: '700px', top: '490px', display: 'flex', alignItems: 'flex-end', height: '85px' }}>
                <span style={{ fontSize: '48px', marginRight: '16px', color: 'white' }}>RATING</span>
